...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(
    `/server/dental_management_function/receptionist/hospital/${hospitalId}`
  )
    ...
    .then((data) => {
      ...
      // Once the data is loaded, hide the baller and show the content
      $(".baller-container").hide();
      $(".content-wrapper").show();
    })
    .catch((error) => {
      console.error("Error fetching clinics data:", error);
      // Hide the baller even if there's an error
      $(".baller-container").hide();
      $(".content-wrapper").show();
    });
});
